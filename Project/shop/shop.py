...
def item():
    ...
    if form.validate_on_submit():
        if form.id.data: # it's an update
            try:
                result = DB.update("UPDATE IS601_S_Items set name = %s, description = %s, stock = %s, cost = %s, WHERE id = %s",
                form.name.data, form.description.data, form.stock.data, form.cost.data, form.id.data)
                if result.status:
            ...
            try:
                result = DB.update("""INSERT INTO IS601_S_Items (name, description, stock, cost) 
                VALUES (%s,%s,%s,%s)""",
                form.name.data, form.description.data, form.stock.data, form.cost.data)
                if result.status:
...
def items():
    rows = []
    try:
        result = DB.selectAll("SELECT id, name, description, stock, cost FROM IS601_S_Items LIMIT 25",)
